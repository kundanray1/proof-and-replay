...
The canvas is the primary surface. Drag it with a mouse or pointer, use the wheel or a pinch gesture to zoom, and use `Fit` to restore the whole path. The persistent left sidebar expands from sessions into prompt cycles, workflows, and nested agents; the four view modes remain available as top tabs. Live nodes append without relaying out earlier steps. When nothing is selected, the canvas follows the newest node without resetting zoom; selecting a node pauses auto-follow, frames its complete connected path, and mutes unrelated graph content. Parameters, return types, data fields, workflow ownership, and recorded changes appear in collision-aware context bubbles; use **Clear focus** or double-click empty canvas space to return to the live-following map.
